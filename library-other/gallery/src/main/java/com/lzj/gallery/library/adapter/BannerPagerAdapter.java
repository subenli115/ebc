...
public class BannerPagerAdapter extends PagerAdapter {
    ...
    public void LoadImage(String url, ImageView imageview) {
        if (mRoundCorners == -1) {
            Glide.with(mContext)
                    .load(url)
                    .apply((RequestOptions.placeholderOf(defaultImg).error(defaultImg).dontAnimate().centerCrop().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                    ))
                    .into(imageview);
        } else {
            Glide.with(mContext)
                    .load(url)
                    .apply((RequestOptions.placeholderOf(defaultImg).error(defaultImg).dontAnimate().centerCrop().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).transform(new RoundedCorners(mRoundCorners))
                    ))
                    .into(imageview);
        }
    }
